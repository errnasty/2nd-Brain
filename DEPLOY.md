...
- **GitHub Actions cron reports `HTTP 301` + `APP_URL redirects to: …`** → `APP_URL` is missing its scheme, or uses `http://`. A bare host like `your-app.up.railway.app` makes curl default to `http://`, the host 301s to `https://`, and that scheme change costs the `Authorization` header. The workflows now force `https://` themselves, so this should self-correct, but set the secret to the full `https://your-app.up.railway.app` anyway.

  The redirect target prints as `https://***/api/…` because `APP_URL` is stored as an Actions **secret**, and GitHub masks a secret's value anywhere it appears in a log — including inside a URL it is a substring of. The `***` *is* your host; it is not a second, hidden domain.
